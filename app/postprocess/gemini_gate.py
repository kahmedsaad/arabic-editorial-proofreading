...
# High / medium / low risk confidence floors
_CONF_HIGH = 0.75
_CONF_MEDIUM = 0.85
_CONF_LOW = 0.92
...
def _confidence_threshold(category: str, severity: Severity) -> float:
    ...
    if cat in _HIGH_RISK or severity in {Severity.HIGH, Severity.CRITICAL}:
        return _CONF_HIGH
    if cat in _MEDIUM_RISK or severity == Severity.MEDIUM:
        return _CONF_MEDIUM
    return _CONF_LOW
...
def validate_gemini_finding(
    ...
    *,
    ...
    thr = _confidence_threshold(finding.category, finding.severity)
    ...
    if _inside_quotes(segment.text, finding.start_offset, finding.end_offset):
        ...
        if finding.category in {"attribution", "quote_voice"} and finding.confidence < 0.9:
